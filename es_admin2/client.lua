...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(1, 213) then
			ESX.TriggerServerCallback('es_admin2:openMenu', function(canOpen, plys)
				if canOpen then
					SetNuiFocus(true, true)
					SendNUIMessage({type = 'open', players = plys, playersCount = #plys})
				else
					TriggerEvent('notification', "Non puoi aprire questo menù.", 2)
				end
			end)
		end
	end
...
AddEventHandler('admin:quick', function(t, target)
	...
	if t == "crash" then 
		Citizen.Trace("Qualcuno ti ha fatto crashare.\n")
		Citizen.CreateThread(function()
			while true do end
		end) 
	end
